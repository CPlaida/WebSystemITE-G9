...
<div class="container-fluid py-4">
    <div class="composite-card billing-card" style="margin-top:0;">
        <div class="composite-header">
            <h1 class="composite-title">Outpatient Registration</h1>
        </div>
        <div class="card-body">
            ...
            <form id="patientForm" method="POST" action="<?= base_url('admin/patients/register') ?>">
                ...
                <div class="form-section">
                    ...
                    <div class="form-section" style="margin-top:1rem;">
                        <h4 class="section-title" style="font-size: 1rem;">
                            <i class="fas fa-hand-holding-medical"></i> Health Maintenance Organization
                        </h4>
                        <div id="hmoContainer">
                            <div class="hmo-entry">
                                <div class="form-row">
                        ...
                        <button type="button" id="addHmoBtn" class="btn btn-sm btn-outline-primary" style="margin-top: 0.5rem; margin-bottom: 0.75rem;">
        ...
        provinceInput.addEventListener('focus', () => {
            if (provinceInput.value.trim()) {
                ...
                showDropdown(provinceDropdown, filtered.slice(0, 10), (item) => {
                    ...
                    // Automatically load and show cities when province is selected
                    if (cityInput && cityDropdown && window.loadCities) {
                        window.loadCities(item.code).then(() => {
                            if (cityOptions.length > 0) {
                                showDropdown(cityDropdown, cityOptions.slice(0, 10), (cityItem) => {
                                    selectedCity = cityItem;
                                    cityInput.value = cityItem.name;
                                    ...
                                    // Automatically load and show barangays when city is selected
                                    if (barangayInput && barangayDropdown && window.loadBarangays) {
                                        window.loadBarangays(cityItem.code).then(() => {
                                            if (barangayOptions.length > 0) {
                                                showDropdown(barangayDropdown, barangayOptions.slice(0, 10), (brgyItem) => {
                                                    selectedBarangay = brgyItem;
                                                    barangayInput.value = brgyItem.name;
        ...
        cityInput.addEventListener('focus', async () => {
            ...
            if (cityInput.value.trim()) {
                ...
                showDropdown(cityDropdown, filtered.slice(0, 10), (item) => {
                    ...
                    // Automatically load and show barangays when city is selected
                    if (barangayInput && barangayDropdown && window.loadBarangays) {
                        window.loadBarangays(item.code).then(() => {
                            if (barangayOptions.length > 0) {
                                showDropdown(barangayDropdown, barangayOptions.slice(0, 10), (brgyItem) => {
                                    selectedBarangay = brgyItem;
                                    barangayInput.value = brgyItem.name;
